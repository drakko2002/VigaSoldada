...
def violations(parameters):
    ...
    return np.array([
        ...
        max(0.0, (P * 1000) - (Pc * 10000)),
        max(0.0, delta - 0.25),
        max(0.0, parameters[0] - parameters[3])
    ])

def Calculate_Penalty(parameters):
    penalty = (np.multiply(violations(parameters), 9999999)).sum()
    penalty = penalty * 10000
    return penalty

def Cost_Minimize(parameters):
...
modelList.append(PSO.OriginalPSO(epoch=ME, pop_size=PS, c1=2.05, c2=2.05, w_min=0.7, w_max=0.8))
